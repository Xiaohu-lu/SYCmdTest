...
#define		SFC_CE			0xC7//0x60		// Chip Erase
...
***************************************************************************/
static void SQI_MpuWrEnable(UINT32 nOffsetAddr);        //add static, mark by footman
...
UINT8 SQI_BlockErase(UINT8 Command,UINT32 Address)
{
	...
	SQI_WREN();
	// 2. Force CS to low
	GpioRegs.GPADATA.all=(GpioRegs.GPADATA.all&0xffefffff)|0x00100000;
	GpioRegs.GPAUSE2.all=(GpioRegs.GPAUSE2.all&0xfff0ffff)|0x00080000;
	WaitSqiCycles(10);
	GpioRegs.GPADATA.all=(GpioRegs.GPADATA.all&0xffefffff)|0x00000000;
	...
	// 4. Force CS to High
	GpioRegs.GPADATA.all=(GpioRegs.GPADATA.all&0xffefffff)|0x00100000;
	GpioRegs.GPAUSE2.all=(GpioRegs.GPAUSE2.all&0x0000ffff)|0x11110000;
	WaitSqiCycles(32);
	// 5. Clear INST
	SqiRegs.INST.all = 0;
...
void SQI_SetQOFastRead(void)
{
	...
	status_reg = SQI_RDSR();
	if((status_reg&(1<<SFSR_QE))==0) 
		SQI_WRSR(status_reg|(1<<SFSR_QE));
...
UINT8 SQI_Init(UINT8 clk_div, UINT8 read_mode)
{
	...
	ScmRegs.MODCLKCTRL |= 1<<ID_SQI;	//模块时钟与复位
	ScmRegs.MODRSTCTRL |= 1<<ID_SQI;
		
	// 2. Select ports for SQI
	if((read_mode==QIQOFR) || (read_mode==QIQWOFR) || (read_mode==SIQOFR))		// modify by coins
	{
		GpioRegs.GPAUSE2.bit.GPA20=MUX_SQI;	//CS
		GpioRegs.GPAUSE2.bit.GPA21=MUX_SQI;	//Clk
		GpioRegs.GPAUSE2.bit.GPA22=MUX_SQI;	//IO0
		GpioRegs.GPAUSE2.bit.GPA23=MUX_SQI;	//IO1
		GpioRegs.GPAUSE3.bit.GPA24=MUX_SQI;	//IO2
		GpioRegs.GPAUSE3.bit.GPA25=MUX_SQI;	//IO3
	}	
	else
	{
		GpioRegs.GPAUSE2.bit.GPA20=MUX_SQI;	//CS
	...
	if(clk_div<2)
	{
		ScmRegs.CLKOUTCTRL.all &= ~(UINT32)(1<<23);
		gSqiClkDiv = 1;
	}
	else
	{
		ScmRegs.CLKOUTCTRL.all &= ~(UINT32)(3<<23);
		ScmRegs.CLKOUTCTRL.all &= ~(UINT32)(0x7f<<16);
		ScmRegs.CLKOUTCTRL.all |= (1<<24) | ((clk_div/2-1)<<16) | (1<<23);
		gSqiClkDiv = clk_div;					  //SQI分频
	}	 
